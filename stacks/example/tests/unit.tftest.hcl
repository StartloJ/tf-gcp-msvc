...
run "plan_stack_compute_address" {
  command = plan
  ...
  assert {
    ...
    error_message = "GKE load balancer IP address resource name incorrect"
  }
}

run "plan_stack_region" {
  command = plan
  ...
  assert {
    ...
    error_message = "Compute address should be in asia-southeast1 region"
  }
}
