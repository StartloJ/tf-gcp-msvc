...
variable "local_subnet" {
  description = "local subnet ip range in CIDR format - x.x.x.x/x"
  type        = list(string)
  default     = ["0.0.0.0/0"]
}
...
variable "route_type" {
  type        = string
  description = "Type of route apply to the VPN tunnel. Option only support \"route-based\" or \"policy-based\""
  default     = "route-based"

  validation {
    condition     = contains(["route-based", "policy-based"], var.route_type)
    error_message = "Valid values for route_type are \"route-based\" or \"policy-based\"."
  }
}
