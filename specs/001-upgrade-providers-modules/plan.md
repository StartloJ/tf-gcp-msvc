...
emulator stack for integration tests). All changes must pass `terraform validate`,
`terraform test`, pre-commit hooks, and the integration test suite before being
marked done.
...
+ 1 integration test dir + 2 test runner scripts
...
| **VI. Observable & Self-Documenting** | PASS | `terraform-docs` README regenerated for any module with variable/output changes |
...
└── example/
    ...
    └── tests/
        ├── unit.tftest.hcl        # NEW: stack-level mock tests
